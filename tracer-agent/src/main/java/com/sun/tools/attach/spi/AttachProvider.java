    ...
    public static List<AttachProvider> providers() {
        synchronized (lock) {
            if (providers == null) {
                ...
                // If no providers found via ServiceLoader, add the built-in
                // LinuxAttachProvider. This ensures the attach functionality
                // works on JDK 8 without needing tools.jar on the classpath.
                if (providers.isEmpty()) {
                    try {
                        Class<?> linuxProviderClass = Class.forName(
                            "sun.tools.attach.LinuxAttachProvider", true,
                            AttachProvider.class.getClassLoader());
                        AttachProvider linuxProvider =
                            (AttachProvider) linuxProviderClass.newInstance();
                        providers.add(linuxProvider);
                    } catch (Throwable t) {
                        if (t instanceof ThreadDeath) {
                            ThreadDeath td = (ThreadDeath)t;
                            throw td;
                        }
                        // Not on Linux or class not available - skip
                    }
                }
            }
